    ...
    @section('supercoco')
        <section
            ...
            <div class="max-w-3xl order-2 md:order-1 text-center md:text-left w-[80%]">
                <h2
                    class="text-balance text-4xl lg:text-6xl font-bold tracking-wide leading-tight drop-shadow-md text-green-800 dark:text-white">
                    {{ $superPages['headline']->title ?? 'No Data' }}
                </h2>
                <p class="mt-4 text-lg md:text-2xl text-green-800 drop-shadow-md dark:text-white">
                    {{ $superPages['subheadline']->title ?? 'No Data' }}
                </p>

                <div class="py-4 w-max mx-auto md:mx-0 animate-minimal-bounce">
                    <div type="button"
                        class="group transition-all duration-200 hover:-translate-y-2 text-white rounded-lg bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 shadow-lg shadow-green-500/50 dark:shadow-lg dark:shdow-green-800/80 px-5 py-2.5 text-center me-2 mb-2 flex flex-row items-center gap-1 cursor-pointer ">
                        <div
            ...
            <div class="order-1 md:order-2">
                <img src="{{ asset('Supercoco Products.webp') }}" alt="Supercoco Prodcuct"
                    class="w-full max-w-xl rounded-lg">
            </div>
        </section>
    @endsection

    @include('layouts.sections.supercoco-body')
    @include('layouts.sections.supercoco-featured-product')
    @include('layouts.sections.supercoco-choose')
    @include('layouts.sections.supercoco-bottom')
